import {endAt, limitToLast, onValue, orderByKey, query, ref, set} from "firebase/database";
...
export function getAllMessages(updateMessageState, lastKey, updateLastKey) {
    const messageRef = ref(db, databaseRoutes.MESSAGES);
...
export async function addMessage(message, author = 'Phillibus') {
    ...
    const newMessageRef = ref(db, `${databaseRoutes.MESSAGES}/${timeStamp}`);
    const requestBody = ({
        "USERNAME": author,
        "MESSAGE": message,
        "TIMESTAMP": timeStamp
