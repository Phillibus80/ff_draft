 ...
 * @param {boolean} isRunningFlag - boolean value to set IS_RUNNING
 * @param {boolean} isStartOfDraft - a flag to indicate if the event is the beginning of the draft
 * @return {Promise<void>}
 */
export const setLeagueDraftIsRunning = async (leagueName, isRunningFlag, isStartOfDraft = false) => {
    const timeStamp = Date.now();
    const requestBody = isStartOfDraft
        ? ({
    ...
    try {
        await update(
            ref(database, `/${databaseRoutes.LEAGUES}/${leagueName}/DRAFT/`),
            requestBody
        );
    } catch (e) {
