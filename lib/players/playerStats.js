...
import {databaseRoutes, PLAYER_KEYS, POSITION_ENUM, PREVIOUS_YEAR} from "@/app-constants.js";
 ...
 * @return {Array<draftPlayerConfig>|[]} - array of sorted players.  Can return an empty array.
 */
 ...
 *
 * @param {POSITION_ENUM|string} position - a string of the position.  Will be checked if it is one of the POSITION_ENUM types.
 * @param {number} seasonYear - the year to use when getting the draft-able players.
 * @return {Promise<Array<draftPlayerConfig>>} - a promise that, when successful, will resolve with sorted players data array.
 */
export const getAllPlayers = async (position = '', seasonYear = PREVIOUS_YEAR) => {
    const isRealPos = POSITION_ENUM.some(pos => pos === position);
